...
$(document).ready(function(){
	...
	$('.view-display-id-calendar_1 .view-item-daily_giveaway_calendar .view-data-node-title a, .view-display-id-calendar_1 .view-item-daily_giveaway_calendar .view-data-node-data-field-giveaway-date-field-giveaway-image-fid a').click(function(event) {
	$('.hover').hide();
	$('.calendar-popup').hide();
	var url = $(this).attr('href');
		$.ajax({
			...
			success: function(data) {
				var $fullContent = $('#content-area .content', data);
				var html = $fullContent.html();
	$('<div>').html(html).wrapAll('<div class="calendar-popup"></div>').addClass('hover').parent().css({position:"absolute", top:event.pageY-200, left:event.pageX-200}).click(function(event) {$(this).fadeOut('slow');}).appendTo('body');
	}
	});
	return false;
