...
This class contains all matrix calculations to perform a transform on a given x,y,z point.
You would create 2 classes in your program of this type: Transform camera and Transform active.
**********************************************/
...
public:
    ...
    void setTrans(double M[4][4], double M1[4][4]);  
};

Transform::Transform()
{
}
void Transform::ApplyTransform(double x, double y, double z, bool TFtype, double x1, double y1, double z1)
{
...
void Transform::MultiplyTransform(double tfM[4][4], double M1[4][4],double M2[4][4])
{
    double TempV=0;
...
void Transform::setTrans(double M[4][4], double M1[4][4])
{
	for(int y=0;y<=3;y++)
	{
		 for(int x=0;x<=3;x++)
...
void Transform::BuildElementaryTransform(double tfM[4][4],int tfCode, double tfVal)
{
    double tempM[4][4], resultM[4][4];
    DefineElementaryTransform(tempM, tfCode, tfVal);
    MultiplyTransform(resultM,tfM,tempM);
    setTrans(tfM, resultM);   //Initializes trans member to values calculated above
}
double Transform::DefineCameraTransform(double Fx, double Fy, double Fz, double theta, double phi, double alpha, double r)
{
   ...
   setTrans(trans, tfM);  //Initializes trans member to values calculated above
}

double **Transform::DefineElementaryTransform(double M[4][4], int tfCode, double Mval)
{
